...
class BaseGamePlugin:
    ...
    def validate_user_and_balance(self, cache, sender, avatar_url, amount):
        ...
        if not user:
            return None, None, f"Invalid user: {sender}"
        
        balance_before = user["balance"]
        
        if amount > balance_before:
            return None, None, f"Cannot afford bet: {amount} (balance: {balance_before})"
    ...
    def generate_animation(self, base_animation_path, user_id, user, user_info_before, user_info_after, animated=True):
