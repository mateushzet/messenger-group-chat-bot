CREATE VIEW Plinko_specific_medium_statistic_view AS 
SELECT 
    main.user_name,
    ...
    COALESCE(hit_counts.Number_of_0x7_hits, 0) AS Number_of_0x7_hits,
    ...
    COALESCE(hit_counts.Number_of_3x2_hits, 0) AS Number_of_3x2_hits,
    COALESCE(hit_counts.Number_of_5x6_hits, 0) AS Number_of_5x6_hits,
    COALESCE(hit_counts.Number_of_x12_hits, 0) AS Number_of_x12_hits,
    COALESCE(hit_counts.Number_of_x60_hits, 0) AS Number_of_x60_hits
FROM game_history AS main
LEFT JOIN (
    SELECT user_name, SUM(result_amount) AS Total_winnings_from_plinko
    FROM game_history
    WHERE result_amount > 0 AND game_type = 'Plinko' AND bet_command LIKE '%m%'
    GROUP BY user_name
...
LEFT JOIN (
    SELECT user_name, 
        ...
        COUNT(CASE WHEN note LIKE '0.7' THEN 1 END) AS Number_of_0x7_hits,
        ...
        COUNT(CASE WHEN note LIKE '3.2' THEN 1 END) AS Number_of_3x2_hits,
        COUNT(CASE WHEN note LIKE '5.6' THEN 1 END) AS Number_of_5x6_hits,
        COUNT(CASE WHEN note LIKE '12.0' THEN 1 END) AS Number_of_x12_hits,
        COUNT(CASE WHEN note LIKE '60.0' THEN 1 END) AS Number_of_x60_hits
    FROM game_history
    WHERE game_type = 'Plinko' AND bet_command LIKE '%m%'
    GROUP BY user_name
) hit_counts ON main.user_name = hit_counts.user_name
WHERE main.game_type = 'Plinko' AND main.bet_command LIKE '%m%'
